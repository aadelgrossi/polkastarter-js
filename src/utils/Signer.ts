...
class Signer {
	 ...
	 */
    generateSignerAccount = async ({ password, entropy }: GenerateSignerAccountArgs) => {
	 ...
	 */
     getAddressFromAccount = ({ accountJson, password }: AddressFromAccountArgs) => {
    ...
    signAddresses = async ({
        ...
        contractAddress,
        decimals }: SignAddressesArgs) => {
        const signer = ethers.Wallet.fromEncryptedJsonSync(
            accountJson,
            password
        );

        return await this.signAddressesWithSigner({
            addresses,
            accountMaxAllocations,
            contractAddress,
            decimals,
    ...
    signAddressesWithSigner = async ({
        ...
        addresses.map(async (address, index) => {
            ...
            const result = await this._trySignAddress(signer, address, allocation, contractAddress);
            if (result) {
    ...
    verifySignature = async ({
        ...
        contractAddress,
        signerAddress }: VerifySignatureArgs) => {
        try {
    ...
    signAddress = async ({
        ...
        address,
        accountMaxAllocation,
        contractAddress}: SignAddressArgs) => {
        const message = this._addressToBytes32(address, accountMaxAllocation, contractAddress)
        return await this.signMessage({ signer, message });
    }
