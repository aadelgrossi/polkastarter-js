...
let signs = [];
context('NFT Contract With Whitelisting', () => {
  ...
  it(
    'should deploy Fixed NFT Swap Contract with whitelist',
    mochaAsync(async () => {
      /* Create Contract */
      ...
      signs = await signer.signAddresses({
        addresses: ['0xe797860acFc4e06C1b2B96197a7dB1dFa518d5eB'],
        accountMaxAllocations: [2],
        decimals: 18,
        contractAddress,
        accountJson: account,
        password: 'test1234',
  ...
  it(
    ...
    mochaAsync(async () => {
      ...
      const isAllowedIndivMaxAmount = tokenPurchaseAmount <= indivMaxAmount;
      expect(isSaleOpen).to.equal(true);
      expect(amount).to.equal(true);
      expect(hasTokensLeft).to.equal(true);
      expect(isAllowedIndivMaxAmount).to.equal(true);
    })
  );
  ...
  it(
    ...
    mochaAsync(async () => {
      ...
      const maxAmount = await swapContract.individualMaximumAmount();
  ...
  it(
    'should do a non atomic swap on the Contract with signature',
    mochaAsync(async () => {
      await forwardTime(5);
      const res = await swapContract.swap({
        tokenAmount: 1,
        categoryId: 1,
        maxAllocation: signs[0].allocation,
  ...
  it(
    'should do a non atomic swap on the Contract after adding address to whitelist',
    mochaAsync(async () => {
      await swapContract.addWhitelistedAddress({
  ...
  it(
    ...
    mochaAsync(async () => {
      ...
      expect(amountPurchase).to.equal(Number(1).noExponents());
      expect(Number(purchase.amountContributed).toFixed(2)).to.equal(
        Number(tradeValue).noExponents()
      );
  ...
  it(
    ...
    mochaAsync(async () => {
      ...
      expect(purchases[0].amount).to.equal(1);
    })
  );
